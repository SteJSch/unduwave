...
def loadBessyIIundulatorList() :
	...
	unduList=load_from_json_list(file=file)
	for el in unduList : 
		if el['name'] == 'bessyII_cpmu20' :
			el.update( {'reprFun' : cpmu20.create_cpmu20 } )
	return unduList

def load_from_json_list(file) :
	with open(file,'r') as target:
		my_list = json.load(target)
	return my_list

def find_undu_representation(name, unduList) :
	repr=None
	fnd=False
	for el in unduList :
		if el['name'] == name :
...
class undulatorObj(_attribute_collection) :
	...
	def setCharacter(self,undulatorCharacter) :
		"""
		tries to get data from the characterization?!
		"""
		self._undulatorCharacter=undulatorCharacter
	...
	def getGeoMatRepres(self) :
		...
		if (self._undulatorGeoMatRepres is None) :
			self._undulatorGeoMatRepres=1
		return self._undulatorGeoMatRepres
	...
	def setWAVE(self,wave) :
		...
		setting energies around exptected 1st harm
		"""
		pass

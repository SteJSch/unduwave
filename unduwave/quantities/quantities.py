...
class quantity :
	...
	def plot_over_3d(self,
		...
		ax.set_xlabel(f'{x_quant._plot_name} [{x_quant._unit}]', fontsize=8)
		ax.set_zlabel(
			zLabelAdd+f'{self._plot_name} [{self._unit}]', 
			fontsize=8,
			labelpad=10,
			)
		ax.tick_params(axis='both', which='major', labelsize=8)
		...
		if not (xCutPlot is None) :
			# ydata=Y_data_intrpltd.flatten()
			# zdata=Z_data_intrpltd.flatten()
			# funs=Funs_intrpltd.flatten()

			# yunique=np.unique(ydata)
			# yvalPlot=None
			# yvalsDist=[]
			# for yval in yunique :
			# 	yvalsDist.append( abs(yval-xCutPlot) )
			# yvalPlot=min(yvalsDist)
			# yvalPlotInd = yvalsDist.index(yvalPlot)
			# yvalPlot=yunique[yvalPlotInd]
			# zPlots=[]
			# funsPlot=[]
			# for indy, yval in enumerate(ydata) :
			# 	if yval==yvalPlot :
			# 		zPlots.append(zdata[indy])
			# 		funsPlot.append(funs[indy])


			ydata=Y_data_intr.flatten()
			zdata=Z_data_intr.flatten()
			Fun_data_intr_list=Fun_data_intr.flatten()
			maxFun=max(Fun_data_intr_list)
			maxFunInd=list(Fun_data_intr_list).index(maxFun)
			zvalPlot=zdata[maxFunInd]

			yPlots=[]
			funsPlot=[]
			for indz, zval in enumerate(zdata) :
				if zval==zvalPlot :
					yPlots.append(ydata[indz])
					funsPlot.append(Fun_data_intr_list[indz])
			...
			if clear:
				plt.clf()
			fig.suptitle(f"Slice at {x_quant._plot_name}={zvalPlot:.4f} {y_quant._unit}", fontsize=12)
			# fig.suptitle(f"Slice at {x_quant._plot_name}={zvalPlot:.2f} {y_quant._unit}", fontsize=12)
			# fig.suptitle(f"Slice at {x_quant._plot_name}={yvalPlot:.2f} {y_quant._unit}", fontsize=12)
			ax = plt.gca()
			nfig=nfig+1

			plt.xlabel(f'{y_quant._plot_name} [{y_quant._unit}]', fontsize=10),
			plt.ylabel(f'{self._plot_name} [{self._unit}]', fontsize=10)

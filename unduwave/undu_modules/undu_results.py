...
class undu_results : 
	...
	def __init__(self,undu_api) :
		"""
		Init undu-result class holding all the results
		"""
	...
	def load_field_map(self) : 
		...
		data = pd.read_csv( self._res_folder_undu+files[0], skiprows=range(0, 3), dtype=object, delim_whitespace=True,header=None)
	...
	def load_field_profile(self) :
		...
		if len(files) < 1 :
			return
		data = pd.read_csv( self._res_folder_undu+files[0], skiprows=3, dtype=object, delim_whitespace=True)
	...
	def load_on_axis_undumag_file(self) : 
		...
		if len(files) < 1 :
			return
		data = pd.read_csv( self._res_folder_undu+files[0], dtype=object, delim_whitespace=True)
	...
	def extract_summary(self) : 
		...
		if not ( self._undu_api._prog_paras.iforce.get() == 0 ) :

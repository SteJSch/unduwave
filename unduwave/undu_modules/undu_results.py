...
class undu_results : 
	...
	def __init__(self,undu_api=None) :
		"""
	...
	def load_from_res_folder(self,add='') :
		...
		self.load_on_axis_undumag_file(add)
		self.load_field_profile(add)
		self.load_field_map(add)
		self.extract_summary(add)

	def load_field_map(self,add='',res_folder=None) : 
		if res_folder is None :
			if self._res_folder_undu is None :
				return
			res_folder=self._res_folder_undu

		file_name = f'{add}.map'
		files = f_h.find_files_exptn(folder = res_folder, hints = [file_name], exptns = [])
		if len(files) > 0 :

			map_bfield=bfield.bfield(
			...
			# 	unit='',
			# 	)
			self._res_quantities = self._res_quantities + [map_bfield]

	def load_field_profile(self,add='') :
		file_name = f'_field_profile{add}.dat'
	...
	def load_on_axis_undumag_file(self,add='') : 
		file_name = f'undumag_on-axis{add}.dat'
		files = f_h.find_files_exptn(folder = self._res_folder_undu, hints = [file_name], exptns = [])
		if len(files) < 1 :
			return
		...
		unduBfield=bfield.bfield(
			unitsXB=[0.001,1.0], # setting the units
			plot_name="bfield",
			name='bfield',
			description='magnetic field on axis',
			unit='T',
			)
		unduBfield.fromQuants(
			xQuant=x_quant,
			byQuant=by_quant,
			bzQuant=None,
			)
		self._res_quantities = self._res_quantities + \
			[x_quant,by_quant,bz_quant,byInt_quant,bzInt_quant,byInt2_quant,bzInt2_quant,unduBfield] 

	def load_force_undumag_file(self,add='') : 
		file_name = f'undumag{add}.frc'
		files = f_h.find_files_exptn(folder = self._res_folder_undu, hints = [file_name], exptns = [])
		if len(files) < 1 :
			return
	...
	def extract_summary(self,add='') : 
		...
		if not ( self._undu_api._prog_paras.calc_force.get() == 0 ) :
			force_components, torque_components = self.load_force_undumag_file(add=add)

...
if nargs > 1:
  try:
    ...
    print("To force total recompilation delete ",n,UI + Sepp + "bin"+Sepp+"undumag_win.exe",n)
    Quit()
  #end try
#endif
...
Scomp = "x86_64-w64-mingw32-gfortran-win32 -std=legacy -c -O2 -cpp -fbacktrace -ffpe-summary=invalid,zero,overflow -fdec -fd-lines-as-comments -Wno-align-commons -fno-automatic -ffixed-line-length-none -finit-local-zero -funroll-loops "
Scomp_nowarn = "x86_64-w64-mingw32-gfortran-win32 -w -std=legacy -c -O2 -cpp -fbacktrace -ffpe-summary=invalid,zero,overflow -fdec -fd-lines-as-comments -Wno-align-commons -fno-automatic -ffixed-line-length-none -finit-local-zero -funroll-loops "
Scomp_all = "x86_64-w64-mingw32-gfortran-win32 -std=legacy -c -O2 -cpp -fcheck=all -fbacktrace -ffpe-summary=invalid,zero,overflow -fdec -fd-lines-as-comments -Wno-align-commons -fno-automatic -ffixed-line-length-none -finit-local-zero -funroll-loops "
Scomp_omp = "x86_64-w64-mingw32-gfortran-win32 -std=legacy -c -O2 -cpp -finit-local-zero -fcheck=all -fopenmp -fbacktrace -ffpe-summary=invalid,zero,overflow -fdec -fd-lines-as-comments -Wno-align-commons -ffixed-line-length-none -funroll-loops "
...
def get_undu_tree():
  ...
  try:
    Texe = os.stat(UI + Sepp + 'bin' + Sepp + 'undumag_win.exe').st_mtime_ns
  except:
    Texe = 0
  #endtry
...
#      #endif
  ...
  if kmain:
    scom = UI + "shell/compile_undumag_incl_win.sh"
    for frm in ['bpolyederf90m.mod','commandlinef90m.mod','undumagf90m.mod']:
      ...
      if Idry == 0:
        ...
        #endtry
    #endfor
    sgfor = "x86_64-w64-mingw32-gfortran-win32 -O2 -cpp -fd-lines-as-comments -Wno-align-commons -fopenmp -fcheck=bounds -ffixed-line-length-none -finit-local-zero  -funroll-loops -o " + UI + "bin" + Sepp + "undumag_win.exe " + UI + "main" + Sepp + "undumag_main.f"
    slink = ' '
    ...
    if Iverbose > 0: print("\n",scom,"\n")
    if Idry == 0: os.system(scom)
    if Iverbose >=0: print("\n--- " + UI  + "bin"+Sepp+"undumag_win.exe updated ---\n")
  else:
    if Iverbose >=0: print("\n--- No need to update " + UI  + "bin"+Sepp+"undumag_win.exe ---\n")
  ...
  for dll in dlls:
    ...
    if Path(path).exists():
      shutil.copy2(path, UI + 'bin' + Sepp + dll)

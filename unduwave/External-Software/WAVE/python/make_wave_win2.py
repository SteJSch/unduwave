...
WI = os.getcwd() + Sepp #+ 'External-Software' + Sepp + 'WAVE'+ Sepp
...
if nargs > 1:
  try:
    ...
    print("To force total recompilation delete ",n,"bin" + Sepp + "wave_win.exe",n)
    Quit()
  #end try
#endif
...
def get_wave_tree():
  ...
  try:
    Texe = os.stat(WI + Sepp + 'bin' + Sepp + 'wave_win.exe').st_mtime_ns
  except:
    Texe = 0
    kmain = 1
  #endtry
...
#      if Iverbose > 0: print("\n",scom,"\n")
  ...
  if kmain:
    ...
    if Iverbose >=0: print("\n--- No need to update " + WI  + "bin" + Sepp + "wave_win.exe ---\n")
  #endif
...
def wave_compile():
  ...
  if Iverbose:
    print("\nMaking .." + Sepp + "bin" + Sepp + "wave_win.exe\n")
...
#  else: forcomp(scom)
  ...
  scom = 'cd ' + pathmod + ' && ' \
  'x86_64-w64-mingw32-gfortran-win32 -c -O2' + \
  ...
  scom = 'cd ' + pathmain + ' && ' \
  'x86_64-w64-mingw32-gfortran-win32 -O2 -cpp' + \
  ' -fopenmp' + \
  ' -fcheck=bounds' + \
  ' -fbacktrace' + \
  ' -ffpe-summary=invalid,zero,overflow' + \
  ...
  ' -static-libgcc' + \
  ' -funroll-loops' + \
  ' -o ..' + Sepp + 'bin' + Sepp + 'wave_win.exe wave_main.f' + \

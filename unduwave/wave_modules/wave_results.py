...
class wave_results : 
	...
	def load_from_res_folder(self,add='') :
		"""
		goes through folder and loads all results it can find, flux, flux_d, brilliance
		"""
		self.find_load_trajectory_bfield_data(add=add)
		self.find_load_power_distribution(add=add)
		self.find_load_flux(add=add)
		self.find_load_brilliance(add=add)
		self.find_load_flux_density_on_axis(add=add)
		self.find_load_stokes(add=add)
		self.extract_summary(add=add)
	...
	def find_load_flux_density_distribution(self, energies,add='') : 
		"""
		...
		filesTmp = f_h.find_files_exptn(folder = self._res_folder_wave, hints = [file_name], exptns = [])
		if len(add)>0:
			files=[]
	...
	def find_load_flux_density_on_axis(self,add='') :
		"""
		Loads on-axis flux-density
		"""
	...
	def find_load_brilliance(self,add='') :
		"""
	...
	def find_load_power_distribution(self,add='') :
		"""
		...
		for filename in os.listdir(self._res_folder_wave):
			if len(add)>0:
				if filename.find(add)<0:
					continue
			if (filename.find(f'WAVE{add}.mhb') >= 0): 
				...
				for ind, line in enumerate(lines) :
					if line.find('2000  3   ! id and kind of histogram of Ntuple') >= 0 :
						indStrt=ind
					if not (indStrt is None) :
						if line.find('! number of entries') >= 0 :
							indStrt=ind+2
							break
				while True :
					if indStrt is None :
						return
					lineNow=lines[indStrt]
					lineNow=lineNow+lines[indStrt+1]
					...
					for el in lineNow :
						if len(el) < 1 :
							continue
						try:
							els.append(float(el.strip()))
						except:
							pdb.set_trace()
					powLines.append(els)
					indStrt=indStrt+3
					if lines[indStrt-1].find('! -----------------') >= 0 :
						break
				powLinesPD=pd.DataFrame(powLines,columns=['x','y','z','pow','powf','iz','iy','iobs','isou'])
				yMin=powLinesPD['y'].min()
				yMax=powLinesPD['y'].max()
				zMin=powLinesPD['z'].min()
				zMax=powLinesPD['z'].max()
				powLinesNew=[]
				for line in powLinesPD.to_dict('records') :
					if not (line['y'] == yMin) :
						if not (line['y'] == yMax) :
							if not (line['z'] == zMin) :
								if not (line['z'] == zMax) :
									powLinesNew.append(line)
				powLinesPD=pd.DataFrame(powLinesNew)
				sources=powLinesPD['isou'].unique()
				powRes=None
				for source in sources : 
					if powRes is None :
						powRes = copy.deepcopy(powLinesPD[powLinesPD['isou'] == source ]).reset_index(drop=True)
						continue
					powResTmp = powLinesPD[powLinesPD['isou'] == source ].reset_index(drop=True)
					powRes['pow']=powRes['pow']+powResTmp['pow']
					powRes['powf']=powRes['powf']+powResTmp['powf']
				powLinesPD=powRes

				z_powerf_d = quantity(
					api=self._wave_api,
					data=powLinesPD['z']*1e3,
					name="powerf_z",
					plot_name="z",
				...
				powerf_d = quantity(
					api=self._wave_api,
					data=powLinesPD['powf']/1e6,
					name="powerf_distribution",
					plot_name='Power Distribution with Emittance',
					description='Total Power Distribution including Emittance Effects',
					unit='$W/mm^2$',
					)
				self._res_quantities = self._res_quantities + [z_powerf_d,y_powerf_d,powerf_d] 
			...
			if (filename.find(f'wave{add}.pow') >= 0): 
				...
				powerg_d = quantity(
					api=self._wave_api,
					data=data['powerGrazing'].to_list(),
					name="powerg_distribution",
					plot_name='Power Distribution',
					description='Total Power Distribution',
					unit='$W/mm^2$',
					)
				powerof_d = quantity(
					api=self._wave_api,
					data=data['powerSpectral_Emit']/1e6,
					name="powerof_distribution",
					plot_name='Power Distribution with Emittance',
					description='Total Power Distribution including Emittance Effects',
					unit='$W/mm^2$',
					)
				self._res_quantities = self._res_quantities + [z_powero_d,y_powero_d,powero_d,powerof_d,powerg_d] 

	def find_load_trajectory_bfield_data(self,add='') :
		"""

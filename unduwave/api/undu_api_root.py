...
class undu_api :
	def __init__(self,undu_mode='from_undu_magns',magn_objects=None) :
		"""
		...
		self.set_magnet_objects(magn_objects=magn_objects)
	...
	def set_force_calc(self,object,segmentations=np.array([20,50,50])) :
		"""
		...
		center_coord=object._absCenter
		self._prog_paras.center_magnet_struct.set(0)

		self._prog_paras.calc_force.set(1) # switching force calc on
		# setting the center coordinates of where to calc the force
		self._prog_paras.force_box_center_x.set(center_coord[0])
		self._prog_paras.force_box_center_y.set(center_coord[1])
		self._prog_paras.force_box_center_z.set(center_coord[2])
		...
		self._prog_paras.force_segm_x.set(segmentations[0])
		self._prog_paras.force_segm_y.set(segmentations[1])
		self._prog_paras.force_segm_z.set(segmentations[2])
	...
	def get_results(self,add='') :
		...
		results.load_from_res_folder(add=add)
		return results

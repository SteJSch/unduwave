...
# 			return self.createEasyMagnet(centerTmp,magnParasTmp,name=name)
# 		return self._magnFun(centerTmp,magnParasTmp,name=name)
...
# 		main_block = undulatorComponents.undu_magnet_block_coords(
# 			p_center=center,
# 			name=name_main,
# 			magnetParameters=magnParas,
# 			mother=name,
# 			)
# 		return undulatorComponents.undu_magnets(magnet_blocks=[main_block])
...
class unduRepresentation() : 
	def __init__( self,
		...
		self._endStructType=endStructType
		if len(self._endSlits[0]) < 1 :
			self._endSlits[0]=[self._glueSlit,self._glueSlit,self._keeperSlit]
		if len(self._endSlits[1]) < 1 :
			self._endSlits[1]=[self._keeperSlit,self._glueSlit,self._glueSlit,self._glueSlit]
	...
	def get_end_magn_paras(self,endNum,magnParas):
		...
		endMagnParas._len_x_main.set(endMagnParas._len_x_main()*shrink)
	...
	def createKeeper(self,
		...
		for indMagn, magnetC in enumerate( self._magnetsPerKeeper ):
			...
			magnetC._magnParas._magnetization.set(magnetizations[(startNum+indMagn)%len(magnetizations)])
			magnetC._magnParas._magn_unit_vec.set(undu_magnets.create_magnetization_unit_vec(
	...
	def createEndStructElliptStd(self,
		...
		# lenEnd=0
		# for endMag in magnets :
		# 	lenEnd=lenEnd+endMag._magnParas._len_x_main()
		...
		newStart=startNum
		for indMagn, magn in enumerate( magnets ):
			...
			magnetC._magnParas._len_x_main.set(magnetL*fact)
			magnet = magnetC.createMagnet(center=magnCenter,name=name)
			magnet_blocks.append(magnet)
	...
	def createEndStructStd(self,
		...
		lenEnd=self.getEndLength(which=which)
		pX=pX-lenEnd/2.0
		endSlitOffset=0
		if which == 'us' :
			endSlits=self._endSlits[0]
		elif which == 'ds' :
			endSlits=self._endSlits[1]
			pX=pX+endSlits[0]
			endSlitOffset=1 #we already used first entry for ds
		magnet_blocks=[]
		for indMagn, magn in enumerate( magnets ):
			magnet=magnets[(indMagn)%len(magnets)]
			magnetC=copy.deepcopy(magnet)
			magnetL=magnetC._magnParas._len_x_main()
			pX=pX+magnetL/2.0
			pY=pY0+magnetC._center._y
			...
			magnet = magnetC.createMagnet(center=magnCenter,name=name)
			magnet_blocks.append(magnet)
			if (indMagn+endSlitOffset) < len(endSlits) :
				pX=pX+magnetL/2.0+endSlits[indMagn+endSlitOffset]
			else :
				pX=pX+magnetL/2.0
		return undu_magnets.undu_magnets(magnet_blocks=magnet_blocks)

	def getEndLength(self,which) :
		"""
		Endpart length including gaps between periodic part
		"""
		length=0
		if which == 'us' :
			for ind, endMag in enumerate(self._endMagnets[0]) :
				length=length+endMag._magnParas._len_x_main()
				length=length+self._endSlits[0][ind]
		elif which == 'ds' :
			for ind, endMag in enumerate(self._endMagnets[1]) :
				length=length+endMag._magnParas._len_x_main()
				length=length+self._endSlits[1][ind]
		return length
	...
	def createRow(self,
		...
		lEndMagns=self._endMagnets[0]
		rEndMagns=self._endMagnets[1]

		lenLend=self.getEndLength(which='us')
		lenRend=self.getEndLength(which='ds')

		endStructLCenter=copy.deepcopy(center)
		endStructLCenter._x=center._x-\
					self._nperiods*self.getPeriodLength()/2.0+\
					self._keeperSlit/2.0-\
					lenLend/2.0
		...
		endStructL=self.createEndStructStd(
		...
		endStructR=self.createEndStructStd(
			...
			which='ds',
			nameCore=name,
			startNum=3+self._nperiods*4,
			)
		# row1=undu_magnets.undu_magnets(magnet_blocks=[nperiods])
		...
		return row1
		# return undu_magnets.undu_magnets(magnet_blocks=[nperiods])	

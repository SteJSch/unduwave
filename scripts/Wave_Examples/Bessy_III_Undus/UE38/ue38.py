...
K_para_z = np.linspace(0.5,4.547,10) # Possible K-Values in horizontal (z) mode
K_para_z=K_para_z[0] 
K_para_y=0.0

K_para_y = np.linspace(0.5,3.88,10)  # Possible K-Values in vertical (y) mode
K_para_z=0.0
K_para_y=K_para_y[0]

K_para_c = np.linspace(0.5,4.16,10)  # Possible K-Values in circular mode
K_para_z=K_para_c[0]
K_para_y=K_para_c[0]

period_length = 0.038 # m
...
undu_paras.elliptUnduB0Y.set(b0_y)
undu_paras.elliptUnduB0Z.set(b0_z)
undu_paras.elliptUnduNumPeriods.set(nperiods)
undu_paras.elliptUnduPerLength.set(period_length)
